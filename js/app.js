        ...
        generateGroceryList: function(xml) {
            app.request({
                ...
                success: function(data, textStatus) {
                    ...
                    var groceries = [];
                    for (var key in gl) {
                        var aisle = gl[key].string[0]['#text'];
                        if (!aisle) aisle = 'Not Specified';
                        var category = gl[key].string[1]['#text'];
                        if (!category) category = '';
                        var description = gl[key].string[2]['#text'];
                        if (!description) description = '';
                        var name = gl[key].string[3]['#text'];
                        if (!name) name = '';
                        var price = gl[key].string[4]['#text'];
                        if (!price) price = '';
                        var quantity = gl[key].string[5]['#text'];
                        if (!quantity) quantity = '';
                        var store = gl[key].string[6]['#text'];
                        if (!store) store = '';
                        var units = gl[key].string[7]['#text'];
                        if (!units) units = '';
